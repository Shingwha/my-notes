...
项目根目录提供了 `start.bat` 脚本，可自动启动本地服务器。
...
您可以将 My Notes 部署到任何静态网站托管服务，最简单的方法是使用 GitHub Pages。
...
| 版本 | 文件 | 特点 |
|------|------|------|
| **新版（推荐）** | `index.html` + `css/` + `js/` | 代码分离，易于维护和自定义 |
| **旧版** | `index_old.html` | 单文件，所有功能集成，开箱即用 |
...
2. **上传文件**：
   - **新版**：将 `index.html`、`css` 文件夹、`js` 文件夹上传到仓库根目录
   - **旧版**：将 `index_old.html` 改名为 `index.html` 后上传到仓库根目录
...
4. **等待部署**：几分钟后 GitHub 会提供访问链接

5. **访问**：打开 `https://<username>.github.io/<仓库名>/`
